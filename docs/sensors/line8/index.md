# 循行者 8 路感應器 (LineSensor8)

<div style="text-align: center; margin-bottom: 2rem;">
  <span style="display:inline-block; background:#ff4500; color:#fff; padding:3px 12px; border-radius:20px; font-family:monospace; margin-bottom:1rem; font-size: 0.85rem;">LNS-08-PRO · 競賽級</span>
  ...
  <p style="font-family: monospace; color: #aaa; letter-spacing: 1px;">
    <strong>8 通道抗干擾光反射陣列</strong><br>
    一鍵校準 · 無視賽道雜訊 · WRO 花俏地圖專用神機
  </p>
</div>
...
WRO 等國際比賽的地圖往往在黑線附近佈滿鮮豔的圖案和顏色，傳統循線感應器非常容易因此誤判，導致機器人「跟著圖案走」而失控。
MBC 8 路循線感應器針對此痛點進行了專項優化，搭載高度抗干擾演算法，能在最花俏複雜的賽道上，精準鎖定黑線、無視一切旁邊的視覺干擾！

* **🚀 抗干擾競賽級核心**：板載智慧處理晶片，專為高頻控制所設計，能在極瞬時間內完成 8 通道的雜訊濾波與位置解算，讓機器人始終貼緊正確的軌跡。
* **⚡ 零延遲平行採樣架構 (Zero-Latency Sampling)**：採用硬體級資料並行通道技術， 8 路訊號在底層背景獨立、高速採樣，完全不佔用主機通訊帶寬與執行緒。
* **🛡️ WRO 競賽首選佈局**：精密排布 8 組抗干擾紅外線收發陣列（右至左通道 0 ~ 7），無視賽道旁花俏圖案，提供最穩定的軌跡導航。

## 🧠 核心資料特徵與演算法

為了讓您的循線車達到比賽級的極致平滑與穩定，底層硬體會將 8 路原始反射訊號在背景進行即時校準、二值化與連續性濾波，產出以下四個核心數據特徵：
...
<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin: 25px 0;">
  ...
  <div style="background: rgba(255,69,0,0.05); border: 1px solid rgba(255,69,0,0.3); border-radius: 8px; padding: 20px; display: flex; flex-direction: column; justify-content: space-between;">
    <h4 style="color: #ff4500; margin-top: 0; font-size: 1.15rem; margin-bottom: 10px;">📐 特徵線寬 與 二值化圖譜</h4>
    <p style="font-size: 0.95em; margin: 0; line-height: 1.6; color: #ccc;">指出目前觸發的感測器數量（即線寬，可用於精準判斷十字路口、丁字路口或直角），以及 8 顆感測器是否觸發的二值化狀態圖譜，方便還原精細軌跡。</p>
  </div>
  <div style="background: rgba(0,210,255,0.05); border: 1px solid rgba(0,210,255,0.3); border-radius: 8px; padding: 20px; display: flex; flex-direction: column; justify-content: space-between;">
    <h4 style="color: #00d2ff; margin-top: 0; font-size: 1.15rem; margin-bottom: 10px;">🛡️ 高階抗干擾濾波</h4>
    <p style="font-size: 0.95em; margin: 0; line-height: 1.6; color: #ccc;">針對 WRO 複雜地圖設計，系統會即時過濾不連續的雜訊區塊，強制鎖定主賽道黑線，大幅度降低車體因周邊圖案干擾而失控掉線的風險。</p>
  </div>
</div>
...
<div style="display: flex; flex-direction: column; gap: 12px; margin: 20px 0;">
  ...
  <div style="display: flex; align-items: center; gap: 15px; background: rgba(0, 210, 255, 0.06); border: 1px solid rgba(0, 210, 255, 0.25); border-radius: 8px; padding: 12px 15px;">
    ...
    <div style="flex: 1;">
      <strong style="color: #00d2ff; font-size: 1.05rem;">板載 8 路狀態 LED 陣列</strong>
      <div style="color: #ddd; font-size: 0.95rem; margin-top: 4px; line-height: 1.5;">
...
<div class="env-hub-grid" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));">
  ...
  <a href="#/sensors/line8/arduino-i2c" class="env-card mcu" style="padding: 40px 20px;">
    <div style="display: flex; justify-content: center; margin-bottom: 20px; min-height: 110px; align-items: flex-end;">
...
為了讓 8 路光電感測器獲得最佳的比對增益，本產品提供了簡便的高效自動校準：
...
- **❓ 為什麼跟著旁邊花俏圖案跑了？**
  → 8 路具備極強的抗干擾濾波機制，但仍需**正確的校準基底**。確認校準時有掃到「最黑的黑線」和「最白的白色區域」。檢查場地光線是否均勻，並確認感應器高度在 0.5 至 1.2 公分。
