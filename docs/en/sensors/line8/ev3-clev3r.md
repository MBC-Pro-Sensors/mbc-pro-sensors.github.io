...
Applicable to **clev3r v1.7.1.0** (BASIC language development environment for EV3). In this environment, the Pathfinder 8-Way Sensor will be perfectly identified by the LEGO system as an official **Infrared Sensor**.
...
Use BASIC commands to read the Beacon mode value list (4 bytes).
...
## <a href="../examples/line8/ev3/line8_ev3_clev3r.bp" target="_blank" data-ignore="true" download>📥 Download Example Programs</a>

<div style="text-align: center; margin: 25px 0;">
  <img src="../images/sensors/line8/ev3-clev3r-example.webp" alt="clev3r Example Code Block Example" style="max-width: 100%; border-radius: 8px; border: 1px solid rgba(10,186,181,0.25); box-shadow: 0 10px 30px rgba(0,0,0,0.5); filter: drop-shadow(0 0 15px rgba(10,186,181,0.15));" />
  <p style="font-size: 0.85rem; color: #888; margin-top: 10px;">💡 The image above demonstrates the classic PID line-following program structure in clev3r (EV3) for the 8-Way Line Follower.</p>
</div>

* <a href="../examples/line8/ev3/line8_ev3_clev3r.bp" target="_blank" download="line8_ev3_clev3r.bp" style="color: #0abab5; font-weight: bold; text-decoration: none;">📥 clev3r Example (.bp) </a>

---

## 🧩 EV3 Exclusive Subroutines Explanation

To make programming in EV3 clev3r (BASIC) more concise and intuitive, the MBC 8-Way Line Follower provides three powerful "Subroutines". During development, you don't need to install cumbersome extension packs; just call the corresponding function in the main program, and the system will automatically parse the hardware packets in the background and store the latest sensor data in exclusive variables for the motor control logic to use directly.

Here is an introduction to the functions and usage timings of the three core subroutines:

### 1. Basic Line Following Subroutine: `LinePos8_dataUpdate`
...
* **Parameters and Returns**: Input sensor port (`port`), returns variable `pos8`.
...
* **Parameters and Returns**: Input sensor port (`port`), synchronously returns several key variables:
  ...
  * `width`: Real-time line width detection, the value represents the number of currently triggered sensors (range `0` to `8`). Can be used to accurately judge intersections, T-junctions, or the finish stop line.
  * `ch1` / `ch8`: Synchronously read the physical light values of the rightmost (`ch1`) and leftmost (`ch8`) sensors (range `0` to `100`), making it convenient for users to adjust advanced strategies such as anti-derailment or intersections.

### 3. Full Array Decoding Subroutine: `LineCh8_dataUpdate`
* **Purpose**: Thoroughly decode the 8 sensors into independent states. Applicable for writing custom special intersection judgment algorithms, triggering monitoring of sensors at specific positions, or drawing the sensor status directly on the screen of the EV3 hub.
* **Parameters and Returns**: Input sensor port (`port`), the subroutine will instantly parse and return the array `light8`. You can obtain the real-time quantized light value of each sensor through the index (`light8[0]` to `light8[7]`) (value range `1` to `9`, 1=darkest, 9=brightest).
