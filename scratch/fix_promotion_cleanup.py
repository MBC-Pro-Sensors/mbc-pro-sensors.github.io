...
for d in directories:
    ...
    for filename in os.listdir(d):
        ...
        # Very greedy removal of ANY promotion blocks to clean up the mess
        # We look for <br> followed by !!! success and remove everything up to the end of the div or EOF
        # This will safely target any variations of the block I inserted.
        while True:
            # We remove one by one to handle multiple insertions
            match = re.search(r'<br>\s*!!! success ".*?(?:</div>\s*</div>\s*</div>|<br>|$)', content, flags=re.DOTALL)
            if not match:
                break
            
            # Extra safety: Make sure it's actually our block by checking if LegoLauXiao or legolaumo is in it
            matched_text = match.group(0)
            if "LegoLauXiao" in matched_text or "legolaumo" in matched_text:
                # If the matched text ended with <br>, we shouldn't consume that <br> if it belongs to something else
                # Actually, our block ends with </div>\n</div>\n</div>.
                # So we just regex substitute the specific pattern that matches our iframes.
                content = content[:match.start()] + content[match.end():]
            else:
                break # Not our block

        # Also cleanup any stray blocks from the first attempt (the text-only ones)
        content = re.sub(r'<br>\s*!!! success ".*?(?:T9bcndBNQvQ|WgacdWLatbk).*?(?=<br>|<style>|$)', '', content, flags=re.DOTALL)
