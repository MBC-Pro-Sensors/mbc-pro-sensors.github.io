...
for root, dirs, files in os.walk(r"d:\Gemini_Antigravity\mbc-pro-sensors.github.io"):
    ...
    for file in files:
        if file.endswith(".md"):
            ...
            # Match the entire block. We will use a regex to match from `<br>\s*!!! success` to the final `</div>`
            c = re.sub(r'<br>\s*!!! success "\U0001f680.*?</div>\s*</div>\s*</div>\s*', '', c, flags=re.DOTALL)
            
            if orig_c != c:
                # We removed the block. Now append the correct one before the final `<style>` or at end.
